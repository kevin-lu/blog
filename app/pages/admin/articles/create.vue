      ...
      >
        ...
        <n-form-item label="Slug" path="slug">
          <n-input
            ...
            placeholder="文章 URL 标识，如：my-first-article"
            :disabled="isEdit"
          />
          <template #feedback>
            发布后不可修改
...
const handleSubmit = async () => {
  try {
    ...
    let response;
    if (isEdit.value) {
      response = await apiClient.put<any>(`/articles/${form.slug}`, {
        title: form.title,
      ...
      response = await apiClient.post<any>('/articles', form);
    }

    if (response.success) {
      notification.success(isEdit.value ? '更新成功' : '创建成功');
      router.push('/admin/articles');
    } else {
      notification.error(isEdit.value ? '更新失败' : '创建失败', response.message);
    }
  } catch (error: any) {
    // 表单验证失败
    if (error.response) {
      notification.error(isEdit.value ? '更新失败' : '创建失败', error.response.data.message);
    }
  } finally {
    saving.value = false;
  }
